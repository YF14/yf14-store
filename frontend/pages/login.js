      ...
      <div className="min-h-screen grid md:grid-cols-2 bg-[#0f0f1a]">
        ...
        <div className={`hidden md:block relative min-h-[420px] ${isRTL ? 'order-2' : 'order-1'}`}>
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-black/20 flex flex-col items-center justify-end pb-16 px-12">
            ...
            <p className="text-white/70 font-display text-base md:text-lg tracking-wide text-center">
              {t.footer.tagline}
            </p>
          </div>
        </div>
        ...
        >
          <div className="w-full max-w-md">
            ...
            <p className="text-brand-purple-light text-[11px] tracking-[0.25em] uppercase font-body mb-3">
              {t.auth.welcomeBack || (isRTL ? 'مرحباً بعودتك' : 'Welcome back')}
            </p>
            <h1 className="font-display text-3xl md:text-4xl font-light text-white tracking-tight mb-2">
              {t.auth.signInTitle}
            </h1>
            <p className="text-white/50 text-sm font-body mb-8">{t.auth.signInSubtitle}</p>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div>
                <div className="flex justify-between mb-2">
                  <label className="block text-[11px] tracking-[0.2em] uppercase text-white/50 font-body">
                    {t.auth.password}
                  </label>
                  <Link
                    href="/forgot-password"
                    className="text-[11px] text-brand-purple hover:text-white transition-colors font-body"
                  >
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="btn-primary w-full mt-6 rounded-lg disabled:opacity-60"
              >
