...
export default function AdminSaleProductsPage() {
  ...
  const {
    ...
    ['admin-sale-products'],
    () =>
      api.get('/products?filter=sale&showAll=1&limit=500&sort=saleSortOrder').then((r) => r.data),
    { enabled: saleOk }
  );
  ...
  const saveOrderMutation = useMutation(
    () =>
      api.put('/products/reorder-sale', {
        orderedProductIds: items.map((p) => p._id),
      }),
    {
      onSuccess: () => {
        queryClient.invalidateQueries(['admin-sale-products']);
        queryClient.invalidateQueries(['products']);
        toast.success(a.saleOrderSaved);
      },
  ...
  if (!user || !canAccessAdmin(user) || !hasAdminPermission(user, 'sales')) return null;
  ...
  return (
    <AdminLayout>
      <NextSeo title={`${a.salesPageTitle} — Admin`} />
      <div
        ...
        <div className="max-w-5xl mx-auto">
            ...
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
              <div>
                <h1 className="text-2xl sm:text-3xl font-bold tracking-tight flex flex-wrap items-center gap-2">
                  <span>{a.salesPageTitle}</span>
                  <span className="text-lg font-normal font-mono" style={{ color: MUTED2 }}>
                    /sale
                  </span>
                </h1>
                <p className="text-sm mt-1" style={{ color: MUTED }}>
                  {a.salesPageSubtitle.replace('{n}', String(items.length))}
                </p>
              </div>
          ...
          {productFormMode && (
            ...
            >
              ...
              <AdminProductForm
                key={productFormMode === 'new' ? 'new-sales' : productFormMode}
                ...
                onSuccess={() => {
                  ...
                  queryClient.invalidateQueries(['admin-sale-product-count']);
                }}
          ...
          <div className="rounded-2xl border overflow-hidden" style={{ backgroundColor: CARD, borderColor: BORDER }}>
            {isLoading ? (
              ...
              <div className="py-16 text-center text-sm px-4" style={{ color: MUTED }}>
                {items.length === 0 ? a.salesNoProducts : a.noSearchResults}
              </div>
            ) : (
