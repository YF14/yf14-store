...
export default function AdminFeaturedProductsPage() {
  ...
  useEffect(() => {
    ...
    else if (!hasAdminPermission(user, 'featured')) router.replace(getDefaultAdminPath(user));
  }, [user, isAuthReady, router]);

  const featuredOk = !!user && canAccessAdmin(user) && hasAdminPermission(user, 'featured');
  ...
  const {
    ...
    () =>
      api.get('/products?filter=featured&showAll=1&limit=500&sort=featuredSortOrder').then((r) => r.data),
    { enabled: featuredOk }
  );
  ...
  const saveOrderMutation = useMutation(
    () =>
      api.put('/products/reorder-featured', {
        orderedProductIds: items.map((p) => p._id),
      }),
    {
      onSuccess: () => {
        queryClient.invalidateQueries(['admin-featured-products']);
        queryClient.invalidateQueries(['products']);
        toast.success(a.featuredOrderSaved);
      },
  ...
  if (!user || !canAccessAdmin(user) || !hasAdminPermission(user, 'featured')) return null;
  ...
  return (
    <AdminLayout>
      <NextSeo title={`${a.featuredPageTitle} — Admin`} />
      <div
        ...
        <div className="max-w-5xl mx-auto">
            ...
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
              <div>
                <h1 className="text-2xl sm:text-3xl font-bold tracking-tight flex flex-wrap items-center gap-2">
                  <span>{a.featuredPageTitle}</span>
                  <span className="text-lg font-normal font-mono" style={{ color: MUTED2 }}>
                    /featured
                  </span>
                </h1>
                <p className="text-sm mt-1" style={{ color: MUTED }}>
                  {a.featuredPageSubtitle.replace('{n}', String(items.length))}
                </p>
              </div>
          ...
          {productFormMode && (
            ...
            >
              ...
              <AdminProductForm
                key={productFormMode === 'new' ? 'new-featured' : productFormMode}
                ...
                onSuccess={() => {
                  ...
                  queryClient.invalidateQueries(['admin-featured-product-count']);
                }}
          ...
          <div className="rounded-2xl border overflow-hidden" style={{ backgroundColor: CARD, borderColor: BORDER }}>
            {isLoading ? (
              ...
              <div className="py-16 text-center text-sm px-4" style={{ color: MUTED }}>
                {items.length === 0 ? a.featuredNoProducts : a.noSearchResults}
              </div>
            ) : (
