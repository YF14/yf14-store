...
export default function OrderDetailPage() {
  ...
  const handleCancel = async () => {
    if (typeof window !== 'undefined' && !window.confirm(t.account.cancelOrderConfirm)) return;
    try {
      await api.post(`/orders/${id}/cancel`);
      toast.success(t.account.orderCancelledToast);
      refetch();
    } catch (err) {
      toast.error(err.response?.data?.error || t.account.cannotCancelOrder);
    }
  };
    ...
    return (
      <Layout>
        <div className="container-luxury py-24 text-center" dir={isRTL ? 'rtl' : 'ltr'}>
          <h1 className="font-display text-4xl mb-4">{t.account.orderNotFound}</h1>
          <Link href="/account/orders" className="btn-primary">{t.account.myOrdersNav}</Link>
        </div>
      </Layout>
    );
  }
  ...
  return (
    <Layout>
      <NextSeo title={`${order.orderNumber}`} />
      <div className="container-luxury py-12" dir={isRTL ? 'rtl' : 'ltr'}>
        <div className="flex flex-wrap items-start justify-between gap-4 mb-10">
          <div>
            <Link href="/account/orders" className="text-sm text-brand-warm-gray hover:text-brand-gold mb-3 inline-block">
              ← {t.account.myOrdersNav}
            </Link>
          ...
          <div className="bg-white border border-brand-black/10 p-6 mb-8">
            <div className="flex items-center justify-between relative">
              ...
              {STATUS_STEPS.map((status, i) => (
                <div key={status} className="relative flex flex-col items-center gap-2 z-10">
                  ...
                  <span className={`text-xs tracking-wider hidden sm:block ${i <= stepIdx ? 'text-brand-black' : 'text-brand-warm-gray'}`}>
                    {statusLabel(status)}
                  </span>
                </div>
              ))}
            ...
            {order.estimatedDelivery && (
              <p className="text-sm text-brand-warm-gray mt-6 text-center">
                {t.account.estimatedDelivery}: {new Date(order.estimatedDelivery).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
              </p>
            )}
            {order.trackingNumber && (
              <p className="text-sm text-center mt-2">
                {t.account.trackingLabel}: <span className="font-medium text-brand-gold">{order.trackingNumber}</span>
                {order.shippingCarrier && <span className="text-brand-warm-gray"> {t.account.viaCarrier} {order.shippingCarrier}</span>}
              </p>
            )}
          </div>
        )}
        ...
        <div className="grid lg:grid-cols-[1fr_360px] gap-8">
          <div>
            <h2 className="font-display text-2xl font-light mb-5">{t.account.itemsHeading}</h2>
          ...
          <div className="space-y-6">
            <div className="border border-brand-black/10 p-5">
              <h3 className="text-xs tracking-widest uppercase mb-4">{t.account.orderSummaryBox}</h3>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between text-brand-warm-gray"><span>{t.account.subtotalLine}</span><span>{formatIQD(order.subtotal)}</span></div>
                <div className="flex justify-between text-brand-warm-gray"><span>{t.account.shippingLine}</span><span>{order.shippingCost === 0 ? t.common.free : formatIQD(order.shippingCost)}</span></div>
                {order.promoDiscount > 0 && (
                  <div className="flex justify-between text-green-600"><span>{t.account.discountLine} ({order.promoCode})</span><span>−{formatIQD(order.promoDiscount)}</span></div>
                )}
                <div className="flex justify-between font-medium pt-3 border-t border-brand-black/10">
                  <span className="text-xs tracking-widest uppercase">{t.account.total}</span>
            ...
            <div className="border border-brand-black/10 p-5">
              <h3 className="text-xs tracking-widest uppercase mb-4">{t.account.shippingAddressBox}</h3>
              <div className="text-sm text-brand-warm-gray space-y-1">
                <p className="text-brand-black font-medium">{addr.firstName} {addr.lastName}</p>
